...
def main():
    ...
    model = TSN(num_class, args.num_segments, 
                args.num_faces, args.modality, 
                base_model=args.arch, consensus_type=args.consensus_type,
                tangent_consensus_type=args.tangent_consensus_type,
                dropout=args.dropout, trn_dropout=args.trn_dropout, attn_dropout=args.attn_dropout, partial_bn=not args.no_partialbn)
    ...
    train_loader = torch.utils.data.DataLoader(
        TangentDataSet(args.train_list,
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    # 
    # input.shape is torch.Size([8, 180, 224, 224])
    # 
...
def read_class_num(file):
    with open(file, 'r') as r:
        lines = r.readlines()
    
    class_count = list()
    for line in lines:
        class_count.append(int(line.strip('\n')))
    return class_count
...
if __name__ == '__main__':
    ...
    main()
    end_time = dt.now()
    print("best_test_prec1: ", best_prec1)
